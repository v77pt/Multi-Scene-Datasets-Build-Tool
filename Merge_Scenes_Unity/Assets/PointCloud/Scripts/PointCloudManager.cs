...
public class PointCloudManager : MonoBehaviour {
	...
	IEnumerator loadOFF(string dPath){
		...
		for (int i = 0; i< numPoints; i++){
			...
			if (!invertYZ)
				points[i] = new Vector3 (float.Parse (buffer[0])*scale, float.Parse (buffer[1])*scale * -1f,float.Parse (buffer[2])*scale) ;
			else
				points[i] = new Vector3 (float.Parse (buffer[0])*scale, float.Parse (buffer[2])*scale,float.Parse (buffer[1])*scale * -1f) ;
			
			if (buffer.Length >= 5 && useRGBColor)
